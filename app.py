...
# APP VERSION
# =========================================================
APP_VERSION = "CryptoOS Hybrid Breakout V3"
...
breakout_tolerance_pct = st.sidebar.slider(
    "Breakout Toleransı (%)",
    min_value=0.0,
    max_value=2.0,
    value=0.3,
...
macro_lookback = st.sidebar.slider(
    "Makro Lookback",
    min_value=150,
    max_value=600,
    value=400,
...
macro_pivot_window = st.sidebar.slider(
    "Makro Pivot Hassasiyeti",
    min_value=4,
    max_value=20,
    value=8,
    step=1,
...
macro_breakout_max_pct = st.sidebar.slider(
    "Makro Breakout Üst Sınır (%)",
    min_value=1.0,
    max_value=10.0,
    value=6.0,
    step=0.5,
        ...
        if data is not None and "symbols" in data:
            ...
            for item in data["symbols"]:
                try:
                    ...
                    continue
            return valid, ""
        last_err = err or "exchangeInfo alınamadı"
    return set(), last_err
...
def fetch_klines(symbol: str, timeframe_key: str):
    interval, limit = TIMEFRAME_MAP[timeframe_key]
    last_err = ""
    ...
    for url in BINANCE_KLINES_URLS:
        ...
        if data is None or not isinstance(data, list) or len(data) == 0:
            last_err = err or "Binance veri dönmedi"
            continue
        ...
        try:
            ...
            if df.empty:
                last_err = "Parse sonrası veri boş"
                continue
...
def compute_btc_context(timeframe_key: str):
    btc_raw, _ = fetch_klines("BTCUSDT", timeframe_key)
    if btc_raw.empty:
        return None
...
def build_peak_candidates(raw_df: pd.DataFrame, lookback: int, pivot_window_value: int):
    ...
    ranked = sorted(
        [(i, float(v)) for i, v in enumerate(highs)],
        key=lambda x: x[1],
        reverse=True
    )
...
def compute_desc_trendline_breakout(
    raw_df: pd.DataFrame,
    lookback: int = 100,
    pivot_window_value: int = 3,
    ...
    downtrend_bias = (
        current_close < ema20_last * 1.03
        or ema20_last <= ema50_last * 1.02
    )
        ...
        for j in range(i + 1, len(peaks)):
            ...
            if best_candidate is None:
                ...
                if candidate["distance_pct"] < best_candidate["distance_pct"]:
                    best_candidate = candidate
                elif candidate["distance_pct"] == best_candidate["distance_pct"]:
                    if candidate["x2"] > best_candidate["x2"]:
                        best_candidate = candidate
    ...
    return {
        ...
        "trend_debug": (
            ...
            f"viol={best_candidate['violations']} | "
            f"p1={best_candidate['x1']},p2={best_candidate['x2']}"
        ),
        "trend_input_bars": len(local_df),
    }
...
def compute_macro_trend_breakout(
    raw_df: pd.DataFrame,
    lookback: int = 400,
    pivot_window_value: int = 8,
    breakout_max_pct: float = 6.0,
):
...
# ANALYZE SYMBOL
# =========================================================
def analyze_symbol(symbol: str, raw_df: pd.DataFrame, ind_df: pd.DataFrame, mode_value: str, btc_context):
    last = ind_df.iloc[-1]
    ...
    if macro_module:
        macro_raw_df, _ = fetch_klines(symbol, MACRO_TIMEFRAME)
    ...
    if trend_breakout["breakout_pct"] is not None:
    ...
    if macro_breakout["macro_trendline_value"] is not None and price > 0:
        ...
        if -1.0 <= macro_distance_pct <= 1.5:
            ...
            macro_proximity_score = 0
            macro_proximity_note = "makro çizgiden uzak"

    score += macro_proximity_score

    # -------------------------
    # COMBINED SETUP QUALITY
    # -------------------------
    ...
    watchlist = (
        50 <= score < 65
        and setup in ["Pullback", "Early Setup", "Strong Trend"]
        and rs_vs_btc > -2.5
    )

    early_entry = (
        44 <= score < 68
        and setup in ["Early Setup", "Pullback", "Range", "Strong Trend"]
        and rel_volume >= 0.75
        and rs_vs_btc >= -4
        and rsi >= 42
    )
    ...
    unit_risk = max(price - stop, 0.0001)
    ...
    if trend_breakout_flag:
        ...
        if trend_breakout["breakout_pct"] is not None and trend_breakout["breakout_pct"] > -1.5:
            reason_text = f"{reason_text} + trend çizgisine yakın"

    if macro_breakout["is_macro_breakout"]:
        reason_text = f"{reason_text} + makro trend kırılımı"
    elif macro_breakout["macro_note"] == "geçerli makro trend yok":
        risk_text = f"{risk_text} + makro yapı zayıf"
    ...
    return {
        ...
        "Score": score,
        "Tier": tier,
        "Entry Trigger": round(entry_trigger, 4),
        "Breakout Trigger": round(breakout_trigger, 4),
        "Stop Trigger": round(stop, 4),
        "Action": action,
...
def run_single():
    ...
    valid_symbols, valid_err = fetch_binance_usdt_symbols()

    if not valid_symbols:
        st.error(f"Binance sembol listesi alınamadı: {valid_err}")
        return

    if symbol not in valid_symbols:
        st.error(f"{symbol} Binance spot USDT listesinde yok.")
        return

    raw_df, err = fetch_klines(symbol, timeframe)
    if raw_df.empty:
    ...
    result = analyze_symbol(symbol, raw_df, ind_df, mode, btc_context)

    if show_trend_debug:
        st.write("DEBUG Trend Note:", result["Trend Note"])
        st.write("DEBUG Trendline Value:", result["Trendline Value"])
        st.write("DEBUG Breakout %:", result["Breakout %"])
        st.write("DEBUG Trend Debug:", result["Trend Debug"])
        st.write("DEBUG Macro Note:", result["Macro Note"])
        st.write("DEBUG Macro Breakout %:", result["Macro Breakout %"])
        st.write("DEBUG Macro Debug:", result["Macro Debug"])
        st.write("DEBUG Fake Breakout Risk:", result["Fake Breakout Risk"])
        st.write("DEBUG Macro Proximity Score:", result["Macro Proximity Score"])
        st.write("DEBUG Combined Setup Score:", result["Combined Setup Score"])
    ...
    c4.metric("ATR %", result["ATR %"])

    st.markdown("---")
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Mikro Breakout", result["Trend Breakout"] if result["Trend Breakout"] else "Yok")
    m2.metric("Mikro Quality", result["Breakout Quality"] if result["Breakout Quality"] else "-")
    m3.metric("Makro Breakout", result["Macro Breakout"] if result["Macro Breakout"] else "Yok")
    m4.metric("Makro Breakout %", result["Macro Breakout %"] if result["Macro Breakout %"] is not None else "-")

    if result["Trend Breakout"] == "🎈" or result["Macro Breakout"] == "🚀":
        st.balloons()
...
def process_symbol(row_dict, timeframe_key, mode_value, btc_context):
    ...
    raw_df, err = fetch_klines(symbol, timeframe_key)
    if raw_df.empty:
        return {
            "Rank": rank,
            "Name": name,
            "Symbol": symbol,
            "Drop Reason": err[:120] if err else "no_data",
            "kind": "dropped",
        }
    ...
    if ind_df.empty:
        return {
            ...
            "Raw Bars": len(raw_df),
            "Indicator Bars": 0,
            "Drop Reason": "indicator_failed",
            "kind": "dropped",
        }

    try:
        result = analyze_symbol(symbol, raw_df, ind_df, mode_value, btc_context)
        return {
            "Rank": rank,
            "Name": name,
            "Symbol": symbol,
...
def run_screener():
    ...
    if not valid_symbols:
        st.warning(f"Binance sembol listesi alınamadı, sabit evren ile devam ediliyor: {valid_err}")
        valid_symbols = set(build_custom_universe()["symbol"].tolist())
    ...
    for _, row in universe_df.iterrows():
        ...
        if row_dict["symbol"] not in valid_symbols:
            coverage["Dropped"] += 1
            rows.append({
                "Rank": row_dict["rank"],
                "Name": row_dict["name"],
                "Symbol": row_dict["symbol"],
                "Drop Reason": "Binance spot USDT listesinde yok",
                "kind": "dropped",
            })
        else:
            coverage["Symbol Match"] += 1
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [
            executor.submit(process_symbol, row_dict, timeframe, mode, btc_context)
    ...
    if not analyzed_df.empty:
        analyzed_df = analyzed_df[analyzed_df["Score"] >= min_score].sort_values("Score", ascending=False)
    ...
    if not analyzed_df.empty:
        ...
        high_quality_combo_df = analyzed_df[
            (analyzed_df["Combined Setup Score"] >= 10)
        ].copy()
    ...
    if not dropped_df.empty:
        render_table(
            dropped_df[[c for c in ["Rank", "Name", "Symbol", "Raw Bars", "Indicator Bars", "Drop Reason"] if c in dropped_df.columns]],
